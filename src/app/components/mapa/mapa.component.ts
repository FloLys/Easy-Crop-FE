...
export class MapComponent implements OnInit {
  ...
  jsonData = [
    { "Row Labels": "October 30, 2024", "Flood": 389.5, "Storms": 2411.5, "Wildfire": 1280.5, "Drought": "", "Frost": "", "": "" },
    { "Row Labels": "November 02, 2024", "Flood": 2089.9, "Storms": 3385.1, "Wildfire": 532.2, "Drought": "", "Frost": "", "": "" },
    { "Row Labels": "November 07, 2024", "Flood": 3451.6, "Storms": 3472.4, "Wildfire": 86.5, "Drought": "", "Frost": "", "": "" },
    { "Row Labels": "November 20, 2024", "Flood": 5326.5, "Storms": 3116.3, "Wildfire": 13.5, "Drought": "", "Frost": "", "": "" },
    { "Row Labels": "November 27, 2024", "Flood": 6511.4, "Storms": 2915.8, "Wildfire": 10.6, "Drought": "", "Frost": "", "": "" },
    { "Row Labels": "December 02, 2024", "Flood": 9377.4, "Storms": 2303.6, "Wildfire": 25.1, "Drought": 1905.3, "Frost": "", "": "" },
    { "Row Labels": "December 10, 2024", "Flood": 9934.7, "Storms": 1905.3, "Wildfire": 36.8, "Drought": 2915.8, "Frost": "", "": "" },
    { "Row Labels": "December 15, 2024", "Flood": 11416, "Storms": 1419.9, "Wildfire": 34, "Drought": 2411.5, "Frost": "", "": "" },
    { "Row Labels": "December 20, 2024", "Flood": 12909.4, "Storms": 363.4, "Wildfire": 27.4, "Drought": 3385.1, "Frost": "", "": "" }
  ];
  ...
  setupChartData() {
    ...
    const floodData = this.jsonData.map(item => this.getNumericValue(item.Flood));
    const stormsData = this.jsonData.map(item => this.getNumericValue(item.Storms));
    const wildfireData = this.jsonData.map(item => this.getNumericValue(item.Wildfire));
    const droughtData = this.jsonData.map(item => this.getNumericValue(item.Drought));
    ...
    const normalizedFloodData = floodData.map(value => (value / maxFlood) * 100);
    const normalizedStormsData = stormsData.map(value => (value / maxStorms) * 100);
    const normalizedWildfireData = wildfireData.map(value => (value / maxWildfire) * 100);
    const normalizedDroughtData = droughtData.map(value => (value / maxDrought) * 100);
  ...
  getCrops() {
    this.http.get<Crop[]>('http://34.69.23.77/plants_data').subscribe(
      (data: Crop[]) => {
  ...
  onSubmit() {
    if (this.selectedLat && this.selectedLng && this.weatherData && this.selectedCrop) {
      ...
      if (selectedCropData) {
        ...
        console.log('Propiedades del cultivo seleccionado:', Object.keys(selectedCropData));
      } else {
      ...
      const payload = {
        ...
        weather: {
          temperature_2m: this.weatherData.data.find((d: any) => d.parameter === 't_2m:C')?.coordinates[0].dates[0].value,
          maxTemperature: this.weatherData.data.find((d: any) => d.parameter === 't_max_2m_24h:C')?.coordinates[0].dates[0].value,
          minTemperature: this.weatherData.data.find((d: any) => d.parameter === 't_min_2m_24h:C')?.coordinates[0].dates[0].value,
          windDirection: this.weatherData.data.find((d: any) => d.parameter === 'wind_dir_10m:d')?.coordinates[0].dates[0].value,
          pressure: this.weatherData.data.find((d: any) => d.parameter === 'msl_pressure:hPa')?.coordinates[0].dates[0].value,
          precip1h: this.weatherData.data.find((d: any) => d.parameter === 'precip_1h:mm')?.coordinates[0].dates[0].value,
          precip24h: this.weatherData.data.find((d: any) => d.parameter === 'precip_24h:mm')?.coordinates[0].dates[0].value,
          sunrise: this.weatherData.data.find((d: any) => d.parameter === 'sunrise:sql')?.coordinates[0].dates[0].value,
          sunset: this.weatherData.data.find((d: any) => d.parameter === 'sunset:sql')?.coordinates[0].dates[0].value
        },
  ...
  updateCurrentMarker() {
    ...
    this.currentMarker = L.marker([this.currentLat, this.currentLng]).addTo(this.map)
